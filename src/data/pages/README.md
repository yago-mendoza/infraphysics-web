...
**Essay share cards.** Optional `shareCard: cover` uses the article cover as a shaded background with the existing grid. Omit the field for the generic card; without a cover it also falls back to the generic card. Translations inherit this field from the English article. Cards are generated by `npm run og` or the pre-commit hook; the content build then includes their URLs in the social metadata.
