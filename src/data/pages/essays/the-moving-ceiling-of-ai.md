...
complexity: 4
featured: true
related: ["3841205", "1147290", "9150483"]
hidden: false
---

In 2020 I was trying to make a base language model useful at work, before language models had learned how to behave like assistants. I had a problem (a loading algorithm for cargo ships carrying luxury vehicles across the Atlantic, combinatorial optimization with enough interdependencies to make it genuinely unpleasant) and I was using GPT-3 to help draft the documentation.

Not to solve the problem. The models couldn't do that. But I'd found something: if you embedded a question inside a text block and engineered the surrounding context carefully enough, the most probable completion was the kind of answer you wanted. It wasn't a conversation. It was distribution engineering. I paired the prompt with a regex filter that rejected repetitive completions and kept the output inside a usable format. Context shaped the distribution; ordinary code screened the samples.

I didn't have good language for what I was doing. In retrospect it was a crude combination of prompt construction, rejection sampling and deterministic validation, assembled before the current post-training and agent tooling existed. At the time it just felt like a trick that worked more often than it should. What stayed with me was the intuition underneath it: a useful language-model system is not only a model. It is also the machinery that shapes, tests and accepts its outputs.
...
Code is different. Code executes. A test passes or fails. A program compiles or it doesn't. You get a binary reward signal, automatic, scalable and cheap to evaluate. This is an industrialized version of the intuition I was learning manually in 2020: generation becomes much more useful when something outside the model can reject bad candidates. The difference is enormous. My regex checked surface form after one completion; an RL loop can turn verified outcomes into a training signal across millions of attempts.
