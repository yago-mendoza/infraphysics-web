---
slug: forecasting-visual-auras
id: "7654321"
displayTitle: "Forecasting visual auras from event streams: a simulation-first study"
subtitle: "A sparse model, an LSTM, and what honest validation would let you claim"
category: projects
date: "2026-06-26"
thumbnail: https://cdn.infraphysics.net/articles/7654321/cover.webp
...
status: ongoing
tags: ["ML","time series","healthcare","simulation","evaluation"]
complexity: 5
technologies: [Python, PyTorch, NumPy, JavaScript]
tldr: "A simulation-first assay, with no clinical seizure predictor and no empirical claim at the end of it: build a plausible event stream, put a forecasting pipeline under scarcity, and ask what kind of signal survives honest validation. In a deliberately learnable world, a sparse model beats chance modestly (ROC about 0.72), a heavier LSTM fails to clear it, and the bootstrap interval on its edge over a dumb threshold crosses zero. Above chance in the simulator, and nowhere near usable in the world."
---
...
I am going to avoid pretending this is a neurology lecture. I am not a neurologist, and for this project the exact boundary between *[migraine aura](https://ichd-3.org/1-migraine/1-2-migraine-with-aura/)*, *[epileptic aura](https://www.epilepsydiagnosis.org/seizure/sensory-overview.html)*, **cortical spreading depression**, **inhibition**, **excitation**, **GABA**, **glutamate**, etc. is beside the point.
...
In statistical terms this is close to a [[HwkP17xq|Hawkes-process problem]]: a sequence of events where each event can temporarily raise the probability of the next one. The bet is that **EPVs** are weakly self-exciting. One event makes the next slightly more likely, the sequence gets denser, and the system moves closer to a transition.
...
A rehearsal sounds like a demotion, and it is the useful version of the project, because a rare-event predictor fails in boring ways long before it fails in interesting ones. It leaks across time. It learns a session-specific artifact and calls it physiology. It leans on a plausible feature because the feature *sounds* like a trigger. It reports [[RcAu12xl|ROC-AUC]] as if that were a warning system. It adds a neural network because the word *sequence* appears in the problem statement. So the first thing built is the little world in which the predictor is allowed to embarrass itself.
...
The literature keeps demonstrating why (the papers are at the end; here I only need their headline). Serious seizure-forecasting work is full of models that beat a random predictor in one setting and quietly deflate in another. Temporal models *can* find signal in EEG, and that signal is patient-specific, validation-sensitive, and far easier to overstate than to use. [[rK1Dy2Fa|LSTMs]] are fine. The validation protocol carries almost as much weight as the architecture, and it is the part that keeps getting skipped.
...
![Overlaid session trajectories on a dark background: cyan and orange lines tracing paths through a recent-density versus accumulated-load space.|The risk map: each simulated session as a trajectory through the density-load space; a few climb toward the danger corner.](https://cdn.infraphysics.net/articles/7654321/figures/risk-map.webp "center")
![A phase portrait of the density-and-load dynamics in cyan and orange, showing how the two state variables push each other around.|The phase portrait: how the fast clock (recent density) and the slow clock (accumulated load) push each other around.](https://cdn.infraphysics.net/articles/7654321/figures/risk-phase.webp "center")

The risk map and [[PeYZGshp|phase portrait]] summarize the latent state. The raw material underneath them is the event stream itself, one session per row:

![An event raster of EPV streams, one session per row, ticks marking each flash, stacked across many simulated days.|The event raster: EPV streams stacked session by session, so bursts and quiet stretches line up across days.](https://cdn.infraphysics.net/articles/7654321/figures/risk-raster.webp "center")
...
![The dual clock|The two clocks reading one EPV stream (top), with six more sessions for scale (bottom). Each tick is an EPV; the fast clock (recency-weighted density, cyan) spikes and decays, while the slow clock of accumulated load (amber) creeps up underneath. Aura sessions climb into a racha before the red bar; calm ones never do. The figure draws the hypothesis rather than evidence for it: that auras follow a racha is built into the simulator, so what you see is the assumption the model is asked to recover.](https://cdn.infraphysics.net/articles/7654321/figures/clock.webp "center")

![Every window placed by recent EPV density on the x-axis and accumulated session load on the y-axis; the rare positive windows form a bright cluster up and to the right.|The session scatter: every window placed by its two clocks. The rare positive windows are the bright cluster drifting up and to the right, exactly the corner the model has to find.](https://cdn.infraphysics.net/articles/7654321/figures/risk-scatter.webp "center")
...
In rare-event work, [[FtSl09xi|feature selection]] is debt control. Every extra variable is a loan against evidence you probably do not have, and in a simulation study it is worse, because extra features are extra assumptions with column names.
...
{bkqt/keyconcept}
*Why the split is by session.* In a [[TsrsP0bb|time series]], [[DtLk14xn|data leakage]] does not need to look like cheating. It can look like a clean random split. But if near-duplicate windows from the same session appear on both sides, the model has already seen the shape of the test case. Session-level splitting is the minimum condition for the metric to mean anything, well short of conservative.
{/bkqt}
...
The verdict lands on this evidence regime and on nothing else (LSTMs, brains and aura forecasting all walk away unjudged). It is the cleanest result in the project because it flatters nobody and because it *travels*: at this sample size, capacity buys [[obTC4dWy|overfitting]] rather than skill, which is a fact about the volume of evidence and would hold in any domain.
...
[Payne et al.](https://arxiv.org/abs/2309.09471) used long-term intracranial EEG from the NeuroVista dataset, passed one-minute segments through CNN/LSTM models, and forecast seizure-onset windows across several horizons. Performance was above a random predictor, but it varied patient to patient: better for some, worse for others. [Chambers et al.](https://doi.org/10.3389/fnins.2024.1472747) later tested LSTMs directly on unprocessed intracranial EEG, again reporting above-chance prediction under a carefully defined framework rather than a solved problem.
...
- Payne, D. E., Chambers, J. D., Burkitt, A. N., Cook, M. J., Kuhlman, L., Freestone, D. R., & Grayden, D. B. (2023). *[Epileptic seizure forecasting with long short-term memory (LSTM) neural networks](https://arxiv.org/abs/2309.09471)*. arXiv:2309.09471.
- Chambers, J. D., Cook, M. J., Burkitt, A. N., & Grayden, D. B. (2024). *[Using Long Short-Term Memory (LSTM) recurrent neural networks to classify unprocessed EEG for seizure prediction](https://doi.org/10.3389/fnins.2024.1472747)*. Frontiers in Neuroscience, 18, 1472747.
- Bernabeu, A., Zhuang, J., & Mateu, J. (2025). *[Spatio-Temporal Hawkes Point Processes: A Review](https://doi.org/10.1007/s13253-024-00653-7)*. Journal of Agricultural, Biological and Environmental Statistics, 30, 89–119.
- Reinhart, A. (2018). *[A Review of Self-Exciting Spatio-Temporal Point Processes and Their Applications](https://doi.org/10.1214/17-STS629)*. Statistical Science, 33(3), 299–318.
