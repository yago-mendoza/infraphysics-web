---
slug: building-infraphysics
id: "6184744"
displayTitle: "Building this site: a markdown compiler, a theme system and a second brain"
subtitle: "Written by someone who is not a web developer"
category: projects
date: "2026-03-08"
thumbnail: https://cdn.infraphysics.net/articles/6184744/cover.webp
...
status: deployed
tags: ["React.js","Vite","Markdown","knowledge management","web dev"]
complexity: 4
technologies: [React, TypeScript, Vite, Tailwind CSS, Shiki, marked]
github: https://github.com/yago-mendoza/infraphysics-web
featured: true
tldr: "This is the website you're reading right now. The article, the compiler that rendered it, the theme system coloring it, the wiki-links connecting it. All of it is the project. I've been collecting notes on how things work since 2016. Notebooks, TXTs, physical drawings, Post-its. AI finally made it possible to structure everything. That's the real reason this site exists. The interesting part isn't the CSS or the menus. It's the Second Brain, a knowledge graph where every concept is a node, everything connects, and Claude Code helps me organize ten years of scattered knowledge into something navigable. I considered Astro, I considered MDX, I considered buying a theme. I built everything from scratch instead, because the point was never to have a website. It was to find out what I didn't know."
---
...
I spent 18 days building a custom React app with its own [[E9olQ6Ox|markdown compiler]], a wiki-style knowledge graph I call the Second Brain, and a dual-theme system. In fact, this article is compiled by the same pipeline it describes. The wiki-links in the text point to fieldnotes that were compiled in the same build.

>> 26.03.14 - One thing worth clarifying: although the site runs on React, **the content you're reading was not generated by your browser.** Every article is compiled to HTML at build time, transformed into static HTML, and served as-is. Your browser just sits down and eats.


# The stack

>> 26.01.21 - The cursor is blinking. I am not blinking.
>> 26.01.24 - Built a horizontal navbar. It already feels wrong. Too flat.
>> 26.01.25 - Rip out the navbar. Spend the whole day rebuilding as a sidebar. Break every layout assumption. Worth it.

That gap between knowing _what_ you want to build and knowing _how_ is wider than it looks from the outside.
...
{bkqt/danger}
*In Hindsight.* Tailwind via CDN instead of the npm package means no build-time purging. The CSS payload is larger than it needs to be. And the first implementation was brutal: hardcoded colors everywhere. `text-white`, `bg-gray-900`, `border-gray-700`. Doing the theme migration "later" cost me a full day. Should have set up CSS custom properties on day one. I knew it. I did it anyway.
{/bkqt}

## Why not MDX

I tried MDX for an afternoon. It lets you import React components directly into markdown, which is appealing until prose starts reading like JSX. What I actually needed was smaller: ordinary Markdown for emphasis and links, plus a few structural extensions for `[[wiki-links]]`, typed notes, contextual updates, mathematics and optional multi-path sections. The decision was practical: the article source should remain text first, and every custom token has to earn the maintenance surface it creates.
...
I needed a way to write articles in Markdown and get styled HTML. Standard requirement. The useful additions turned out to be superscripts, subscripts, keyboard shortcuts, typed notes, wiki-links to my knowledge graph, context annotations, mathematics and a small number of richer layout primitives.

Standard Markdown does not cover all of that. So instead of using something like remark/rehype with plugins, I started with `marked` and kept adding layers. Each extension was trivial in isolation. Together they formed a custom language with a much larger interaction surface than any individual regex suggested. The later lesson was subtraction: bold and italic should stay Markdown, links should share one taxonomy, and presentational tricks should disappear when structure can do the job.

The key insight was **ordering**. A structured block has to be isolated before Markdown parses its interior, while code and mathematics must be protected from every textual transform. Reverse two stages and harmless characters become syntax. Obvious in hindsight, difficult to predict until the pipeline has enough interacting parts.
...
The pipeline still has several stages ^[protect code and math, compile typed notes, restore protected text, process links, definition and alphabetical lists, contextual annotations, parse Markdown, highlight code and resolve footnotes and graph references.], but the count is no longer a point of pride. What matters is that the compiler originally built for articles also powers the Second Brain and the localhost editor. One engine is valuable; one engine with fewer special cases is much more valuable.

## Inventing a syntax

Once you start adding custom syntax, you can't stop.

It began with presentational shortcuts: explicit colours, custom underline, category accent text, decorative separators. Each was "just one more regex." Together they had to coexist with Markdown, `[[wiki-links]]`, code, mathematics and one another. Most were eventually removed. Emphasis returned to `**bold**` and `*italic*`; destination semantics moved into links; section rhythm returned to headings and prose. The compiler became more capable by supporting fewer ways to say the same thing.

At some point I had several competing highlight syntaxes. They had bugs, edge cases, and nothing to justify existing alongside bold. Removing all of them was better than selecting a winner. Killing a feature you built is a specific kind of satisfaction.
...
The site has two worlds: lab (dark, projects and Second Brain) and blog (light, essays and tutorials). Four content categories, each with its own accent color: **projects**, **essays**, **bits2bricks**, **fieldnotes**.
...
>> 26.01.30 - Dark/light works. Auto-switch on route. Smooth manual toggle. The hardcoded color migration takes the whole morning but the result is clean.
...
>> 26.02.03 - The thing about collecting notes for ten years is that eventually the collection is the problem. You have hundreds of files across dozens of formats and you can feel how they connect: the structure is in your head, you just can't grep it. The whole point of this project is to take that latent graph and externalize it into something you can actually navigate.
...
> More on how extended thinking changes the game: [[essays/6616933|Every time we think we've found the ceiling, the ceiling leaves]].
...
The second difference is **trailing refs**. Obsidian has backlinks: if you mention a note anywhere in another note's body, it shows up as an automatic backlink. Useful, but noisy. Every casual mention counts the same as a deliberate connection. A throwaway comparison between a CPU and a GPU is treated like a carefully curated relationship. The Second Brain separates these: body mentions are body mentions (tracked, but lightweight), and trailing refs are _intentional connections_, written at the end of a note, with optional annotations explaining the relationship. A deliberate link to [[Z9W6rweD|core]], annotated as “shares execution resources,” tells you _why_ two ideas are connected, not just _that_ they are. And the system only needs the ref on one side. It crosses automatically to the other. Write it once, see it on both pages.
...
{bkqt/tip}
*On building what exists.* I could have used Obsidian. Or Logseq. Or Roam. Any of them would have been faster to set up and better maintained than anything I'd build alone. But none of them would have taught me what I learned by building it: that a graph is more than its nodes, that references need stable identities, that the difference between a mention and a connection is the difference between noise and signal. Sometimes the point of building something that exists is finding out _why_ it exists the way it does, and where it could exist differently.
{/bkqt}
...
The calendar isn't about productivity tracking. It's about **mental archaeology**. When I filter by a two-week window, I see what I was focused on during that period. Not what I _think_ I was focused on, but what I _actually_ wrote about. And the gaps between periods reveal what I missed.
...
The force-directed graph was originally a visual flex. I'll be honest about that. I'd been playing with [[essays/7733621|graph visualization libraries]] for years without going anywhere, because I didn't have enough structured data to make it interesting. Now I do.
...
**English and Spanish.** The essays in Spanish are for my circle: friends, family, university classmates. I send them direct links. The rest is English because if you're going to put something on the internet, you might as well make it readable by anyone interested. I'm a **divulgador** ^[Spanish for someone who makes complex topics accessible. "Science communicator" is close but too formal. "Explainer" is too casual. Divulgador is the right word, and it doesn't have an English equivalent.], but I'm starting small. This site is the small. The circle that reads the Spanish essays is the proof of concept. The English content is the scale.
...
>> 26.03.10 - llms.txt was manually maintained for days. every new article required editing it by hand, and it was already out of sync when I checked. moved it to auto-generation in the build pipeline. the fieldnote count, the article listings, the URLs -- all derived from the same source of truth now. one less thing to forget.
