...
--projects-- was obvious. --threads-- came from systems programming (multiple execution paths sharing a context) and probably from Instagram Threads being everywhere that month. the name stuck because each thread is one train of thought connecting to others. --bits2bricks-- was the hardest — it's about bridging software and physical engineering, taking what i've learned in bits back to the world of ^[as opposed to software, which obeys whatever the developer felt like that morning.]. the name is silly. it stays. --fieldnotes-- came from field notebooks — the kind engineers carry on-site. each one is a concept, a node in a graph.
...
and there was a naming problem. the site already had a ^[a persistent navigation panel on the left.] — the main navigation. now the Second Brain had its own panel too. is it a "manager"? a "browser"? another "sidebar"? i come from systems, where a "panel" is a physical thing on a rack and a "browser" is what you use to read datasheets. i ended up calling it `SecondBrainSidebar` in code. descriptive if not elegant.
...
>> 26.02.17 - optimization pass at 251 notes. three changes: (1) sync cache path — if a note's HTML is already cached, content loads in the same render frame instead of waiting for an async round-trip. navigation between previously visited notes is now instant. (2) prefetching — when you open a note, all visible targets (connections, mentions, neighbors, wiki-links) are fetched in the background. by the time you click, the content is already there. (3) O(N) neighborhood — the old algorithm scanned all notes to find siblings/children for each note (O(N^{2}), ~63k comparisons at 251 notes). replaced with a pre-built map: one pass to bucket notes by parent, then O(1) lookups. also swapped linear `.find()` calls for `Map.get()` in ancestor lookups and island topology. net result: clicking a search result no longer flashes the old note (~100% fix), navigation between connected notes feels instant when prefetched (~90% faster), and index initialization dropped from O(N^{2}) to O(N) in the neighborhood step (~95% reduction at current scale, matters more as notes grow).
