...
MRP is O(n·m) for n products and m time periods — not NP-hard, but the constraint propagation gets interesting when you add timing windows, rounding heuristics (transfers tend to be round amounts — 500, 1000, 1850 — not exact purchase costs), and the global invariant that --no account can go negative at any point in the simulation--. That last constraint is what turns it from an accounting exercise into a validation problem.
