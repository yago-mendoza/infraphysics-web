...
![A four-panel diagnostic dashboard of the simulated world on a dark background, cyan and orange traces: a risk map of overlaid session trajectories, an event raster of EPV streams stacked session by session, a phase portrait of the density-and-load dynamics, and a session scatter spreading every window by recent density and accumulated load.|Four views of the toy world. The risk map and phase portrait trace how density and load evolve; the raster stacks the EPV streams session by session; and the session scatter spreads every window across its two clocks: the rare positive windows are the bright cluster drifting up and to the right.](https://cdn.infraphysics.net/7654321-fig-risk-grid.png "center")
