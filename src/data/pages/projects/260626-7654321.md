...
![Overlaid session trajectories on a dark background: cyan and orange lines tracing paths through a recent-density versus accumulated-load space.|The risk map: each line is one simulated session, tracing how recent density and accumulated load evolve over a day. Most stay low; a few climb toward the danger corner.](https://cdn.infraphysics.net/7654321-fig-risk-map.png "center")

![An event raster of EPV streams, one session per row, ticks marking each flash, stacked across many simulated days.|The event raster: EPV streams stacked one session per row, so bursts and quiet stretches line up across days.](https://cdn.infraphysics.net/7654321-fig-risk-raster.png "center")

![A phase portrait of the density-and-load dynamics in cyan and orange, showing how the two state variables push each other around.|The phase portrait: how the fast clock (recent density) and the slow clock (accumulated load) push each other around over time.](https://cdn.infraphysics.net/7654321-fig-risk-phase.png "center")
