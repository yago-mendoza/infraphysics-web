...
const Sidebar: React.FC = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    return posts
      ...
      .filter(p =>
        (p.displayTitle || p.title).toLowerCase().includes(query) ||
  ...
  const CollapsibleSection = ({
    ...
    const isSectionActive = isActive(`/${category}`);
    ...
    return (
      <div>
        <div className="flex items-center">
          ...
          <Link
            to={`/${category}`}
            className={`flex-grow py-1.5 text-sm transition-colors ${
              isSectionActive ? colorClass + ' font-medium' : 'text-gray-600 hover:text-black'
            }`}
          >
            {label}
          </Link>
  ...
  return (
    <aside className="hidden md:flex flex-col w-56 flex-shrink-0 sticky top-0 h-screen py-6 px-4 border-r border-gray-100 bg-white z-40">
              ...
              <div className="max-h-48 overflow-y-auto">
                {searchResults.map(post => (
                  <Link
                    ...
                    className="block px-3 py-2 text-xs hover:bg-gray-50 border-b border-gray-50 last:border-0"
                  >
                    <span className="text-gray-400 uppercase text-[10px]">{post.category}</span>
                    <div className="text-gray-700 truncate">{post.displayTitle || post.title}</div>
                  </Link>
                ))}
              </div>
            )}
      ...
      <Link
        to="/home"
        className={`flex items-center gap-2 px-2 py-1.5 text-sm rounded transition-colors mb-3 ${
          isActive('/home') ? 'bg-gray-100 text-black font-medium' : 'text-gray-500 hover:bg-gray-100'
        }`}
      >
        <HomeIcon />
        <span>Home</span>
      ...
      <nav className="flex flex-col gap-0.5">
        <CollapsibleSection category="projects" label="projects" colorClass="text-emerald-600" />
        <CollapsibleSection category="threads" label="threads" colorClass="text-amber-600" />
        <CollapsibleSection category="bits2bricks" label="bits2bricks" colorClass="text-blue-600" />
      </nav>
      ...
      <Link
        ...
        className={`flex items-center gap-2 px-2 py-1.5 text-sm rounded transition-colors ${
          isActive('/second-brain') ? 'bg-gray-100 text-black font-medium' : 'text-gray-500 hover:bg-gray-100'
        }`}
      >
...
// 5. Second Brain View (formerly Field Notes)
...
// Sidebar width constant (w-56 = 14rem = 224px)
