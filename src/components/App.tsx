...
const Sidebar: React.FC = () => {
  ...
  // Colors: icon #9AA0A6, text #E8EAED (off-white)
  ...
  const NavItem = ({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) => (
    <Link
      ...
      className={`flex items-center gap-2.5 px-3 py-2 text-sm transition-colors ${
        isActive(to) ? 'font-medium' : 'opacity-80 hover:opacity-100'
      }`}
      style={{ color: isActive(to) ? activeTextColor : textColor }}
    >
  ...
  return (
    <aside
      className="hidden md:flex flex-col w-32 flex-shrink-0 sticky top-0 h-screen py-6 px-2 z-40"
      ...
      <Link
        to="/home"
        className={`flex flex-col items-center gap-1 py-2 ${isActive("/home") ? 'font-medium' : 'opacity-80 hover:opacity-100'}`}
        style={{ color: isActive("/home") ? activeTextColor : textColor }}>
        <span style={{ color: iconColor }}><HomeIcon /></span>
      ...
      <nav className="flex flex-col gap-2">
        <Link to="/projects" className={`flex flex-col items-center gap-1 py-2 ${isActive("/projects") ? 'font-medium' : 'opacity-80 hover:opacity-100'}`} style={{ color: isActive("/projects") ? activeTextColor : textColor }}>
          <span style={{ color: iconColor }}><GearIcon /></span>
          <span className="text-xs">{'projects'}</span>
        </Link>
        <Link to="/threads" className={`flex flex-col items-center gap-1 py-2 ${isActive("/threads") ? 'font-medium' : 'opacity-80 hover:opacity-100'}`} style={{ color: isActive("/threads") ? activeTextColor : textColor }}>
          <span style={{ color: iconColor }}><ThreadIcon /></span>
          <span className="text-xs">{'threads'}</span>
        </Link>
        <Link to="/bits2bricks" className={`flex flex-col items-center gap-1 py-2 ${isActive("/bits2bricks") ? 'font-medium' : 'opacity-80 hover:opacity-100'}`} style={{ color: isActive("/bits2bricks") ? activeTextColor : textColor }}>
          <span style={{ color: iconColor }}><GradCapIcon /></span>
          <span className="text-xs">{'bits2bricks'}</span>
        </Link>
      </nav>

      <Link
        to="/second-brain"
        className={`absolute bottom-8 left-0 w-full flex flex-col items-center gap-1 py-2 ${
          isActive("/second-brain") ? 'font-medium' : 'opacity-80 hover:opacity-100'
        }`}
        style={{ color: isActive("/second-brain") ? activeTextColor : textColor }}
      >
        <span style={{ color: iconColor }}><DiamondIcon /></span>
...
const MobileNav: React.FC = () => {
  ...
  const NavLink = ({ to, colorClass, children }: { to: string, colorClass: string, children: React.ReactNode }) => (
    <Link
      to={to}
      onClick={() => setIsOpen(false)}
      className={`block py-2 transition-colors duration-200 text-lg ${isActive(to) ? colorClass + ' font-medium' : 'text-gray-400'}`}
    >
      {children}
    </Link>
  );
  ...
  return (
    <div className="md:hidden">
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="absolute top-14 left-0 right-0 bg-white border-b border-gray-200 z-50 px-6 py-6">
          <nav className="flex flex-col gap-1">
            ...
            <NavLink to="/second-brain" colorClass="text-gray-600">second brain</NavLink>
            <div className="pt-4 mt-4 border-t border-gray-100">
              <button
                onClick={handleRandom}
                className="text-left text-sm text-gray-300 font-mono"
              >
                /random
              </button>
            </div>
...
const Footer: React.FC = () => {
  return (
    <footer className="w-full py-10 mt-16 border-t border-gray-200 relative z-20">
      <div className="max-w-4xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center text-xs text-gray-400 gap-4 font-mono">
          <div className="flex items-center gap-3">
            <Logo className="w-5 h-5 text-gray-400" />
            <span>© {new Date().getFullYear()} infraphysics</span>
          </div>
          <a href="mailto:contact@infraphysics.net" className="hover:text-black transition-colors">
...
const SectionView: React.FC<SectionProps> = ({ category, colorClass }) => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <SearchBar onSearch={setQuery} placeholder={`search ${category}...`} />
...
const FieldNotesView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)]">
       ...
       <header className="mb-6 pb-4 border-b border-gray-200 flex-shrink-0">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight lowercase text-black mb-6">
          second brain
        </h1>
          ...
          <div className="h-full overflow-y-auto p-6 md:col-span-2 bg-white">
            {activePost ? (
              <div>
                 ...
                 <div
                    className="text-xs md:text-sm leading-loose text-gray-800 font-light content-html"
    ...
    <article className="animate-fade-in max-w-3xl">
      <header className="mb-10">
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-4 font-mono">
          <Link to={`/${post.category}`} className={`hover:underline uppercase ${accentClass}`}>
            {post.category}
          </Link>
          <span>·</span>
          <span>{post.date}</span>
        ...
        <h1 className="text-2xl md:text-4xl font-bold lowercase tracking-tight">{post.displayTitle || post.title}</h1>
      </header>
      ...
      <div className="mt-16 pt-8 border-t border-gray-200">
        <h3 className="text-xs font-mono text-gray-400 mb-6">more</h3>
          ...
          {recommendedPosts.map(rec => (
            <Link key={rec.id} to={`/${rec.category}/${rec.id}`} className="group block">
              ...
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-[10px] uppercase text-gray-400 font-mono">{rec.category}</span>
              </div>
              <h4 className="text-sm font-medium leading-tight group-hover:underline">{rec.displayTitle || rec.title}</h4>
            </Link>
          ))}
        </div>
      </div>
    ...
    <div className="flex flex-col gap-12 animate-fade-in">
      ...
      <section className="border-b border-gray-200 pb-8">
        <p className="text-gray-600 leading-relaxed max-w-xl">
          A digital garden exploring the intersection of software entropy, hardware permanence, and minimal logic.
        </p>
        <div className="flex gap-4 mt-4 font-mono text-xs">
          <a href="https://x.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-black transition-colors">x</a>
          <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-black transition-colors">linkedin</a>
          <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-black transition-colors">github</a>
        </div>
      </section>

      {/* Latest post - clean, prominent */}
      {latestPost && (
        <section
        ...
        >
          <div className="flex items-baseline gap-3 mb-3">
            <span className="text-[10px] font-mono text-gray-400 uppercase tracking-wider">latest</span>
            <span className={`text-[10px] font-mono uppercase tracking-wider ${getCategoryColor(latestPost.category)}`}>
              {latestPost.category}
            </span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold leading-tight tracking-tight group-hover:underline underline-offset-4 decoration-1 mb-2">
            {latestPost.displayTitle || latestPost.title}
          </h2>
          <p className="text-sm text-gray-500 line-clamp-2 max-w-2xl">{latestPost.description}</p>
          <span className="text-xs font-mono text-gray-400 mt-3 block">{latestPost.date}</span>
        </section>
      )}
        ...
        <section className="border-t border-gray-200 pt-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recentPosts.map(post => (
              ...
              >
                <div className="w-20 h-14 bg-gray-100 overflow-hidden flex-shrink-0">
                  <img
                    src={post.thumbnail || 'https://via.placeholder.com/150'}
                    alt=""
                ...
                <div className="flex-grow min-w-0">
                  ...
                  <h4 className="text-sm font-medium leading-snug group-hover:underline underline-offset-2 truncate">
...
// Sidebar width constant (w-32 = 8rem = 128px)
