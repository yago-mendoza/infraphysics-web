...
export function Timeline({rows,title,metrics}:{rows:{bucket:string;[key:string]:string|number}[];title:string;metrics:Record<string,string>}) {
  ...
  return <section className="admin-stats__panel"><div className="admin-stats__section-head"><div><h2>{title}</h2><p>UTC · pasa por una barra para ver su valor.</p></div><div className="admin-stats__controls"><label>Métrica<select aria-label={`${title}: métrica`} value={metric} onChange={e=>setMetric(e.target.value)}>{Object.entries(metrics).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label><label>Agrupar<select aria-label={`${title}: intervalo`} value={grain} onChange={e=>setGrain(e.target.value)}>{rows.some(r=>r.bucket.length>10)&&<option value="hour">Hora</option>}<option value="day">Día</option><option value="week">Semana</option><option value="month">Mes</option></select></label></div></div>
    {!bars.length?<p className="admin-stats__empty">No hay observaciones en este periodo.</p>:<><div className="admin-stats__chart" style={{gap:bars.length>100?0:3}} role="img" aria-label={`${metrics[metric]} por ${grain}. ${bars.length} intervalos; máximo ${n(max)}.`}>{bars.map(([key,value])=><div className="admin-stats__column" key={key} title={`${key} · ${n(value)} ${metrics[metric]}`}><i style={{height:`${value/max*100}%`}}/></div>)}</div><div className="admin-stats__axis"><span>{bars[0][0]}</span><span>Máx. {n(max)}</span><span>{bars[bars.length-1][0]}</span></div><details><summary>Ver valores del gráfico</summary><div className="admin-stats__table-scroll"><table><thead><tr><th>Intervalo UTC</th><th>{metrics[metric]}</th></tr></thead><tbody>{bars.map(([key,v])=><tr key={key}><td>{key}</td><td>{n(v)}</td></tr>)}</tbody></table></div></details></>}
  </section>;
}
export function StatsExplorer({data,onQuery,busy}:{data:Exploration;onQuery:(q:ExploreQuery)=>void;busy:boolean}) {
  ...
  return <section id="exploration" className="admin-stats__exploration"><div className="admin-stats__section-head"><div><p className="admin-stats__eyebrow">Exploración</p><h2>Cruza el tráfico con su contexto</h2><p>Aperturas, revisitas y tiempo visible desde {data.since?.slice(0,10)||'la activación'}. Este detalle no se puede reconstruir para visitas anteriores.</p></div></div>
    <form className="admin-stats__panel admin-stats__controls" onSubmit={e=>{e.preventDefault();onQuery({groupBy:second&&second!==first?[first,second]:[first],filters});}}>
      <label>Agrupar por<select aria-label="Agrupar por" value={first} onChange={e=>{setFirst(e.target.value);if(second===e.target.value)setSecond('');}}>{Object.entries(dimensions).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
      <label>Cruzar con<select aria-label="Cruzar con" value={second} onChange={e=>setSecond(e.target.value)}><option value="">Sin segundo grupo</option>{Object.entries(dimensions).filter(([k])=>k!==first).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
      {Object.entries(dimensions).map(([key,name])=><label key={key}>{name}<select aria-label={name} value={filters[key]||''} onChange={e=>setFilters({...filters,[key]:e.target.value})}><option value="">Todos</option>{(data.options[key]||[]).map(value=><option key={value} value={value}>{value}</option>)}</select></label>)}
