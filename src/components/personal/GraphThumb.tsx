// Static picture of the whole wiki graph for the Home spotlight, painted the
// way the wiki's minimised graph looks: dark field, faint typed edges, small
// root-coloured nodes with a soft halo. Positions and colours are precomputed
// at build time (scripts/compute-graph-thumb.js), so this is plain inline SVG:
// no index fetch, no force simulation.

import React from 'react';
import thumb from '../../data/graph-thumb.generated.json';

interface ThumbNode { id: string; x: number; y: number; r: number; c: string }
  ...
  <svg className={className} viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
    ...
    <g className="graph-thumb-links">
      {data.links.map(([a, b, type], index) => {
        ...
        return <line key={index} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={EDGE_STROKES[type] ?? EDGE_STROKES[0]} />;
      })}
    </g>
