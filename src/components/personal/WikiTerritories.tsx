...
import { WIKI_PATH, secondBrainGraphPath, secondBrainPath } from '../../config/categories';
...
export const WikiTerritories: React.FC = () => {
  ...
  return (
    <section className="wb-split" aria-label="Explore all Wiki domains">
      <div ref={container} className="wb-territory-map" onMouseLeave={() => setActive(null)}>
        {partitionAreas(roots, size.width, size.height).map(root => (
          <button key={root.id} type="button"
            ...
            aria-label={`${root.label}: ${root.count} notes, ${share(root)}. Opens the wiki.`}
            onMouseEnter={() => setActive(root.id)} onFocus={() => setActive(root.id)} onClick={() => navigate(WIKI_PATH)}
      ...
      <aside className="wb-detail" id="wb-domain-detail">
        <small>{selected ? 'Selected domain' : 'Read the mosaic'}</small>
