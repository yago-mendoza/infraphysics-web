...
export const EssaysList: React.FC<SectionRendererProps> = ({ posts, query, getExcerpt, getMatchCount }) => {
  ...
  return <div className="essays-editorial-matrix">{posts.map((post, index) => {
    ...
    return <article key={post.id} className="group"><Link to={postPath(post.category, post.id)}>
      ...
      <span className="essays-editorial-copy"><span><time>{formatDateTimeline(post.date)}</time><i>{post.lang || 'en'}</i></span><strong><Highlight text={post.displayTitle || post.title} query={query} /></strong><small><Highlight text={excerpt || post.description} query={query} />{matches > 0 && ` · ${matches} matches`}</small></span>
