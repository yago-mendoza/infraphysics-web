...
export function AdminStatsView() {
  ...
  const load = async (event?:React.FormEvent, nextQuery?:ExploreQuery) => {
    event?.preventDefault();
    const requested=nextQuery||query.current; setBusy(true); setError('');
  ...
  return <main className="admin-stats">
    ...
    <header className="admin-stats__titlebar"><span>INFRAPHYSICS</span><span>Analytics · privado</span></header>
    <div className="admin-stats__body">
    <p className="admin-stats__path">/admin/stats &gt; {report ? 'CONNECTED' : 'AUTH REQUIRED'}</p>
    <h1>Observatorio de tráfico</h1>
    ...
    {report && <>
      ...
      <StatsReport report={report} range={range} busy={busy} onQuery={q=>{void load(undefined,q);}} filters={<form onSubmit={load} className="admin-stats__filters">
        ...
        <button disabled={busy}>{busy?'Consultando…':'Actualizar periodo'}</button>
      </form>} />
    </>}
    </div><div className="admin-stats__status">InfraPhysics · Cloudflare · UTC · {report ? 'READY' : 'LOCKED'}</div>
    </div>
  </main>;
}
