...
Both wiki graphs on  come from , produced by  at the end of every build (seeded 3D layout, identical across builds). The carousel door draws it as inline SVG (); the closing plate renders it once with three.js as lit spheres from a fixed camera (, no animation loop, three loaded on demand). Both copy the root palette and edge colours from ; if those change, update the script and the components. The pinned piece in the carousel is the newest post with .
