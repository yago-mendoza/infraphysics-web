...
- Post detail: `/lab/:category/:id` (dark), `/blog/:category/:id` (light). Blog articles share one geometry (`article-geometry.css`): essays stack the hero under the subtitle line (the date opens that line in the subtitle's italic face, and views, likes and share sit under the index as in projects; an essay without an index keeps the meta row), Bits2Bricks put it beside the title, keep the meta row and number the section index.
