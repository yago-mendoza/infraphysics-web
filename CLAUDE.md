...
- Wiki: `/wiki`, `/wiki/:uid`, and `/wiki/graph`, which opens the console with the graph workspace already expanded (the home wiki banner lands there; closing the workspace navigates to `/wiki`). On phones the expanded graph is a full-screen takeover (`phone` state in `SecondBrainSidebar`), not a strip beside the drawer; there the minimize button sits at the top of the tool rail (`.graph-minimize-rail`, the top-right one is hidden), the flyouts (lenses, copy as image) follow the rail down (`.graph-flyout`) so the edge switches never cover them, and 3D nodes cannot be dragged (coarse pointer). The phone menu (`MobileNav`) opens with language and theme on top and closes from a pill fixed at the bottom right, where the Menu pill was. Legacy `/lab/second-brain/*` URLs redirect here. No top bar: the global nav is hidden and slides in when the pointer nears the bottom edge (`useProximityReveal`, scroll reveal on touch), with a leading Back arrow to the page the reader came from.
