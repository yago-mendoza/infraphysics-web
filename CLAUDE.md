...
| [pages/VOICE.md](src/data/pages/VOICE.md) | **Always consult when writing or editing articles.** How a text sounds once it obeys the hard rules: concrete up, the two-author seam, failure first, rhythm, numbers, certainty, headings by mechanism, the tics table, coining, the final pass. The kill list itself is STYLE.md rule 10. |
...
| [inbox/README.md](src/data/inbox/README.md) | Incoming material: one file per article idea, never compiled. Where a theme that does not belong in the current draft goes. |
...
1b. **Share cards.** Every url's og:image is a card photographed from the site (`src/views/shareCardDesigns.tsx`, preview at `/og` in dev). After publishing, renaming or retitling anything, run `npm run og -- --base <dev server url>` with the dev server up, then `npm run content`: the script regenerates only the cards whose text, cover or design changed, uploads them to R2 and records them in `src/data/og-cards.json`, which the build reads. New pages or playgrounds must be listed in both `src/lib/shareCards.ts` and `scripts/og-cards.js`.
