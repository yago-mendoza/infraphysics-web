...
**⛔ Standing rule, high priority: no em-dashes in prose.** Never use the em-dash (`—`) as a punctuation break in any body text you write for this site. The author strongly dislikes it and it reads as an AI tell. Use a parenthesis (aside), a period (new sentence), or a comma (light pause) instead, and rebuild the sentence so the punctuation fits its meaning. Routine exception: list-style separators (definition lists, tldr bullets, trailing refs). Otherwise only rare, PUNCTUAL exceptions, never by habit. Full guidance in [pages/STYLE.md](src/data/pages/STYLE.md) (rule 3). This also applies to anything else you write (commit messages, UI copy). The other hard rules of the same rank, no double quotes in prose (use italics), footnotes as `^[…]` before the period, and no *not X, Y* reframes, live in [pages/STYLE.md](src/data/pages/STYLE.md). **The arrow `→` (and `->`, `=>`, `⇒`, `↔`) is absolutely forbidden across the whole site**, in every category and every component, including headings, tables, labels, UI copy, wikinotes, the studio and commit messages; the only exceptions are mathematics inside `{math}` or `\( … \)` and real code in a code block. A sequence of steps is a sentence, a list or a `{sequence}` (STYLE.md rule 2). **Bold only on the clause of a sentence that carries its claim**, never on an isolated term, label or noun phrase (STYLE.md rule 12; the build warns on bold spans of three words or fewer). **A list never has fewer than three items**; one or two are a sentence (STYLE.md rule 13, also warned by the build).
...
| [pages/SYNTAX.md](src/data/pages/SYNTAX.md) | All 22 custom syntax features (typed notes, lifted paragraph, parameter sheets, examples, sequences, tabs, lists, math, images…), edge cases. Parameter sheets hold numbers, symbols and flags only; a term with its meaning is a definition list; data shown as it is (input and output, prompt and answers) is an `{example}` (`/split` for a prompt over its candidate answers, `[faded]` for the rejected one); steps in order or two pipelines side by side are a `{sequence}`; two to four alternatives the reader picks between are `{tabs}`; a code block holds only code; never two typed boxes back to back; a tiny table is prose (author's explicit requests) |
...
| [pages/NO-TICS.md](src/data/pages/NO-TICS.md) | The one paragraph of machine residue every text must lose, in either language, for the site and for Twitter alike. Canonical here beside STYLE.md; `scripts/context-pack.js` reads it and nothing else copies it. |
| [_studio/README.md](_studio/README.md) | The workshop next to the site, never compiled: `_inbox/bank/` (the reservoir, one file per idea or saved item, `facts/` under it for dated figures and claims, `motherlode/` for where to look things up), `articles/` (the generated format packs and the queue of topics they feed), `twitter/` (the strategy, the hand-written prompts and their `_format_ctx/`, the queue of what is going out) and `visuals/aesthetics-examples/` (image style references). |
...
Anything kept for later goes to `_studio/`, never to `src/data/pages/`. The reservoir is `_studio/_inbox/bank/`: an idea, a saved tweet, a link, a quote, a take, a meme with its caption is one file in the appropriate bank subfolder (`articles/essays/`, `articles/projects/`, `articles/bits2bricks/`, or `ideas/` if undecided; quotes live in `ideas/quotes/`, and additions belong inside each article category). The folder is the category: never add `kind` to bank frontmatter (schema in `_studio/README.md`, raw text, do not tidy), and a dated piece of information (a figure, a claim, a quote to reuse) goes one level down in `bank/facts/` with `source` and `status: unverified` until checked. A tweet or thread kept for *how it is written* goes to `_studio/twitter/gen_prompts/_format_ctx/examples/` with its pattern named. An image kept for a style wanted on the site (a mood for covers or figures, not an article image) goes to `_studio/visuals/aesthetics-examples/` with one line in that folder's `STYLE.md` (WebP, long side at most 1600 px, under a megabyte); `pages/VISUAL.md` stays the rule for what is published. An image for a tweet sits beside the tweet's file under `_studio/twitter/queue/` or in the bank.

From the bank the material takes two exits, and the asymmetry matters. **To Twitter**, a piece is written finished into `_studio/twitter/queue/` with its date and target. **To an article**, bank items are joined into a topic in `_studio/articles/queue/`, which is an angle, not a draft; the topic carries `bank:` with paths relative to the bank, including the category folder and omitting `.md` and each of those items carries `topics:` back, so traceability runs both ways. `npm run find -- --links` prints that graph and flags one-way links, dead slugs and topics with no source; run it after touching either folder, because nothing enforces the fields at build time.

To find a piece (*something dry about agents that is ready*, *what did I say about MCP*), run `npm run find -- --tag <tag> --status <status>` (schema and filters in `_studio/README.md`; the vocabulary is `_studio/twitter/gen_prompts/_format_ctx/_vocabulary/TAGS.md`, `--tags` lists it with counts; `--tvb <text>` searches the author's hand-written material) instead of reading the folders; when adding a piece, fill the frontmatter completely, always with the date set to the day it is saved, and add any new tag to TAGS.md first. The finder (`scripts/studio-find.js`) is a tool for the agent, not an interface anyone depends on: when a question would be answered faster with a new filter, output or source, change the script and say so, never read the folders by hand instead.

Tweets follow no canon: the site's hard rules are not imposed there, and tone is never a rule in the studio (`mood` is optional description). The one thing both channels share is `pages/NO-TICS.md`. To help with a tweet or thread, use the hand-written prompt for the job in `_studio/twitter/gen_prompts/`, which names what to paste: `_format_ctx/ACCOUNT.md`, the generated `_format_ctx/NO-TICS.md`, `STRATEGY.md`, and the whole of `twitter/queue/` (unposted pieces are live material, one may already be the right answer). **Every tweet job returns three variations**, differing in approach rather than wording, with the one to send named and the profile test applied to it. The author's own material for a voice (expressions, sentences, moves, allergies) is optional colour in `_format_ctx/`, hand-written, one line per entry with tags, only what he adds himself, never scraped from his articles.

For an AI outside this repo writing an article, `_studio/articles/articles-format/write-<category>.md` is the one document to paste: generated by `scripts/context-pack.js` from the authoring docs (`npm run context`, run explicitly), so the docs stay the single source and the packs are never edited by hand. That script also copies `pages/NO-TICS.md` into `_format_ctx/`; those are the only generated files under `_studio/`. To mine the wiki and the articles for tweet or article ideas, the table in `_studio/README.md` says what each layer yields. The build never reads `_studio/`.
...
When the user gives editorial feedback, apply [Where editorial feedback belongs](src/data/pages/README.md#where-editorial-feedback-belongs): capture the reusable lesson in its canonical document according to scope, even when the feedback arrives while working on a particular article or in conversation with an agent.
...
- Wiki: `/wiki`, `/wiki/:uid`, and `/wiki/graph`, which opens the console with the graph workspace already expanded (the Home graph link lands there; Open the Wiki lands on `/wiki`; closing the workspace navigates to `/wiki`). On phones the expanded graph is a full-screen takeover (`phone` state in `SecondBrainSidebar`), not a strip beside the drawer; there the minimize button sits at the top of the tool rail (`.graph-minimize-rail`, the top-right one is hidden), the flyouts (lenses, copy as image) follow the rail down (`.graph-flyout`) so the edge switches never cover them, and nodes cannot be dragged with a coarse pointer in either 2D or 3D. The phone menu (`MobileNav`) opens with language and theme on top and closes from a pill fixed at the bottom right, where the Menu pill was. Legacy `/lab/second-brain/*` URLs redirect here. No top bar: the global nav is hidden and slides in when the pointer nears the bottom edge (`useProximityReveal`, scroll reveal on touch), with a leading Back arrow to the page the reader came from.
- Post detail: `/lab/:category/:id` (dark), `/blog/:category/:id` (light). Blog articles share one geometry (`article-geometry.css`; on phones the index list is hidden, the Sections button carries it, and the rail is one strip with the author at the left and views, like and share at the right): essays stack the hero under the subtitle line (the date opens that line in the subtitle's italic face, and views, likes and share sit under the index as in projects; an essay without an index keeps the meta row), Bits2Bricks put it beside the title, keep the meta row and number the section index.
...
- **Bank categories live in folders.** The studio finder reads bank subfolders recursively; article-topic references include the folder (for example `articles/essays/watts-and-tons`). Do not reintroduce `kind` in bank files. `scripts/studio-find.test.js` covers folder-based retrieval and two-way references.

- **Wiki explorer state is shared.** `useSecondBrainHub` owns query, lenses, selected node and directory disclosure, with snapshots per browser history entry. Enter inside `WikiSearchInput` only blurs; keep its events away from global grid shortcuts. Name uses the leaf name/aliases, Path uses the full address. Never restore separate graph selection or incoming-reference traversal for branch highlights.
- **Mobile graph containment.** The expanded workspace follows `visualViewport`, locks body scrolling and hides the underlying app while its portal is open. Keep toolbar controls inside that viewport. Disable node dragging for coarse pointers in both dimensions without disabling physics. Home and menu Wiki links open `/wiki`; only graph links open `/wiki/graph`.
...
The two rail rows (`.glab-engagement` in `article-geometry.css`, `.pj-engagement` in `project-page.css`, the phone `.pj-engagement-top` carrying both classes) restyle views, like and share from scratch and quiet their hover to `--text-heading`, which beats the accent hover of `.article-share-btn:hover` in `article.css` on equal specificity because those files load later. Share is listed apart, on `--art-accent`: the mark grew under the pointer and stayed grey otherwise, which reads as no feedback. Any state added to these rows has to be written out here or it silently loses the accent.

### Project articles are one geometry, in `project-page.css`
The projects branch of `ArticlePostView` renders the cover as a full-bleed plate (`.pj-plate`, meeting the thin black `.article-topbar` that every article page carries on its top edge, fading into the surface) with the title over a blurred scrim, the `ProjectBrief` strip, a labelled summary, the body beside a sticky numbered index (`#article-toc`, the same DOM-toggled `article-toc-link--active` as the blog) and a facts sheet. On phones the rail (index and facts sheet) is hidden: the plate already says author, status, date, complexity, stack and topics, the Sections button carries the index, and views, like and share sit in the plate under the chips (`.pj-engagement-top`, phone only). The old `article-container` / `article-hero` / `article-notes` markup is gone for projects, so those rules in `article.css` are inert there. Body prose is pinned to 1.02rem/1.76 in the article face (`--font-article`) on desktop; on phones the site-wide 1.08rem rule applies. Because the plate crops the cover, `thumbnailFocus` in the frontmatter should sit on the subject (the auras cover uses 40, not 0). In every geometry the Giscus comments render after the column grid (`.glab-after`, `.pj-after`, both repeating the grid template so they line up with the body column), never inside the body cell: the sticky index is bounded by its grid row, so comments inside the cell would let it ride down over the discussion.
...
The essays type system (the shared article face `--font-article` for body and subtitle, Newsreader title and headings, 40rem column, drop cap, quiet meta row) is one block at the very end of `src/styles/article-layout.css`, using `.article-essays.article-page-wrapper …` selectors with `!important`. It has to sit last and be that specific because `global.css` and earlier `article.css` blocks also pin essay type with `!important`. Editing font sizes, families or the column width for essays anywhere else will silently lose to this block; change it here.
...
### Definition lists, box leads, examples and sequences are compiled DOM, styled in `article.css`
A `- TERM:: text` row compiles to `.defn > .defn-term + .defn-desc` (a two-column grid, no dash, no `<strong>`): CSS written against `.defn strong` matches nothing. The italic first sentence of a typed box is lifted out by `processBlockquoteContent` into `p.bkqt-lead` (the title line); a box whose first paragraph opens with an emphasised word that ends in a period gets that word as its title, so keep leads as sentences. `{example}` and `{sequence}` (`processExamples`, `processSequences` in `compile.js`) are skipped whole by the `[STYLE]` checker in `build-content.js` like `{math}` and `{params}`; a new fence must be added to that regex or its lines are checked as prose. `{tabs}` (`processTabs`) only wraps its panels in HTML blocks with blank lines around them, so the panel bodies stay Markdown for the rest of the pipeline; the panel count (two to four) is enforced through `syntaxErrors` in the compile options. The two runtime behaviours (tab switching, the `.is-wide` measure that drops the guides of a wide example) live in `WikiContent.tsx`, which renders every article and wikinote body; the `{sequence}` lines are pure CSS and every one of them (rail, return segments, corners, node ring) is a 2px border, never a filled box or a box-shadow: on a display scaled to 125% or 150% the browser floors a border's width to whole device pixels but snaps a filled box by its two edges, so a 2px border beside a 2px-wide background renders at a different width (lines that must match use one primitive and whole pixels); paragraphs inside a panel are listed in the direct-child body type rule of `article-layout.css`.
...
Analytics exploration: `/admin/stats` now separates stored public totals without historical estimates, measured daily series and a filtered opening cohort. See `workers/counters/README.md` for deduplication, sample coverage, retention, query limits and measurement costs. The client is `src/lib/siteAnalytics.ts`; verify with `node scripts/site-analytics.test.mjs` plus Worker tests when changing event semantics. Reports default to 30 UTC days (maximum 366 per request); selector options use those dates too. JSON downloads are compact, and SQLite storage size is shown. Hourly-to-daily retention is configured on: seven complete UTC days plus today stay hourly, older data becomes daily in automatic batches. See the counters README for retention and first-release backup.
